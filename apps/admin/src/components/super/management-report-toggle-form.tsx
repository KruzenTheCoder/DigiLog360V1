...
import { AlertCircle, Check, FileText, Loader2, Search, Users } from 'lucide-react';
...
export function ManagementReportToggleForm({
  rows, siteNames,
}: {
  rows: Profile[];
  ...
  const [state, setState] = useState<Record<string, boolean>>(
    Object.fromEntries(rows.map((r) => [r.id, r.can_log_management_report])),
  );
  ...
  async function toggle(id: string, next: boolean) {
    ...
    const { error: e } = await (supabase as any)
      .from('profiles')
      .update({ can_log_management_report: next })
      .eq('id', id);
    if (e) {
  ...
  async function bulkSetVisible(value: boolean) {
    ...
    const { error: e } = await (supabase as any)
      .from('profiles')
      .update({ can_log_management_report: value })
      .in('id', ids);
        ...
        <CardContent className="space-y-3 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>
                <strong>{enabledCount}</strong> with access · {rows.length} total · {filtered.length} shown
              </span>
            </div>
              ...
              <div key={r.id} className="flex flex-wrap items-center gap-4 px-5 py-3">
                <div className="min-w-[220px] flex-1">
                  <p className="flex items-center gap-2 truncate font-semibold">
                    {on && <FileText className="h-4 w-4 text-emerald-500" />}
                    {r.full_name ?? '(no name)'}
...
function ToggleSwitch({
  ...
  return (
    <button
      ...
      disabled={busy}
      aria-pressed={on}
      aria-label={on ? 'Remove Management Reports access' : 'Grant Management Reports access'}
